        ...
        if (len(split_html) == 1):
            ...
            for i in range(1, len(split_child)):
                ...
                for link in part_soup.find_all('a', href=True):
                    link_url = "https://bn.quora.com" + link['href'] + "?share=1"
...
def crawlTopicQuestions(topic_urls):
    ...
    file_question_urls =  open("question_urls.txt", mode = 'wb')
    file_topic_urls = open("topic_urls.txt", mode = 'rb')
...
def crawlQuestionData(file):
    ...
    file_question_urls = open(file, mode = 'rb')
    file_data = open("answers.csv", mode = 'wb')
    file_users = open("users.txt", mode = 'wb')
    ...
    while (current_line):
        ...
        for i in range(1, len(split_html)):
            ...
            if (not user_id):
                continue
            user_id = "https://bn.quora.com" + user_id['href'] + "?share=1"
            ...
            for x in range(1, len(users_voted_raw)):
                ...
                for user in users_voted_soup.find_all('a', href=True):
                    if (users_voted):
                        users_voted += ", " + "https://bn.quora.com" + user['href'] + "?share=1"
                    else:
                        users_voted += "https://bn.quora.com" + user['href'] + "?share=1"
                    file_users.write(("https://bn.quora.com" + user['href'] + "?share=1" + '\n').encode('utf-8'))
...
def crawlUser():
    ...
    bar = open('temp.txt', 'wb').writelines(set(unique_users))
    
    file_users = open("temp.txt", mode='rb')
    ...
    while(current_line):
        ...
        for x in range(1, len(followers_raw)):
            ...
            for follower in followers_soup.find_all('a', href=True):
                count += 1
                if (followers):
                    followers += ", " + "https://bn.quora.com" + follower['href'] + "?share=1"
                else:
                    followers += "https://bn.quora.com" + follower['href'] + "?share=1"
        ...
        for x in range(1, len(following_raw)):
            ...
            for each_following in following_soup.find_all('a', href=True):
                count += 1
                if (following):
                    following += ", " + "https://bn.quora.com" + each_following['href'] + "?share=1"
                else:
                    following += "https://bn.quora.com" + each_following['href'] + "?share=1"
